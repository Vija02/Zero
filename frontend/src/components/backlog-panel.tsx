...
const initialBacklogTasks: BacklogTask[] = [
  ...
  { id: "b6", title: "Auto start synology drive", time: "0:10", tag: "Personal" },
  { id: "b7", title: "Transfer HTB ISA", time: "0:05", tag: "Personal" },
  { id: "b8", title: "Hard drive backup", time: "0:10", tag: "Personal", subtaskCount: "5 subtasks" },
  { id: "b9", title: "Operation Sabina", time: "1:01", tag: "Personal", dueDate: "Jan 16" },
...
export function BacklogPanel() {
  ...
  const { toggleBacklog } = useTaskManager()
  ...
  return (
    <>
      <div className="fixed inset-0 bg-black/50 z-40 sm:hidden" onClick={toggleBacklog} />
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-2 space-y-1 pb-4">
          {tasks.map((task) => (
            <BacklogTaskCard key={task.id} task={task} onToggle={() => toggleComplete(task.id)} />
          ))}
        </div>
      </div>
    </>
...
function BacklogTaskCard({ task, onToggle }: { task: BacklogTask; onToggle: () => void }) {
  ...
  return (
    <div className="bg-[#1a1a1a] rounded-md p-2.5 hover:bg-[#222] cursor-pointer transition-colors border border-transparent hover:border-[#333]">
      <div className="flex items-start justify-between gap-2">
        ...
        {task.time && (
          <span className="text-[10px] text-[#888] bg-[#252525] px-1.5 py-0.5 rounded flex-shrink-0">{task.time}</span>
        )}
      </div>
      ...
      <div className="flex items-center justify-between mt-2">
        <div className="flex items-center gap-1">
          <button onClick={handleCheckClick} className="hover:scale-110 transition-transform">
            {task.completed ? (
          ...
          {task.dueDate && <span className="text-[10px] text-[#666]">{task.dueDate}</span>}
        </div>
        {task.tag && (
